...
class Database():
    ...
    class Worker(ConsumerProducerMixin):
            ...
            try:
                ...
                if key not in self.insert_hijacks_entries:
                    ...
                    self.insert_hijacks_entries[key]['time_detected'] = msg_[
                        'time_detected']
                    self.insert_hijacks_entries[key][
                        'configured_prefix'] = msg_['configured_prefix']
                    self.insert_hijacks_entries[key][
                        'timestamp_of_config'] = msg_['timestamp_of_config']
                else:
        ...
        def handle_resolved_hijack(self, message):
            ...
            try:
                redis_hijack_key = redis_key(raw['prefix'], raw['hijack_as'],
                                             raw['type'])
        ...
        def handle_hijack_seen(self, message):
            ...
            try:
                with get_wo_cursor(self.wo_conn) as db_cur:
                    db_cur.execute('UPDATE hijacks SET seen=%s WHERE key=%s;',
                                   (raw['state'], raw['key']))
                ...
                for hijack_key in raw['keys']:
                    try:
                        ...
                        if seen_action:
                            ...
                            # too
                            if self.redis.sismember('persistent-keys',
                                                    hijack_key):
                            ...
                            with get_wo_cursor(self.wo_conn) as db_cur:
                                db_cur.execute(
                                    query,
                                    (datetime.datetime.now(), hijack_key))
                        else:
            ...
            for withdrawal in self.handle_bgp_withdrawals:
                try:
                    ...
                    for entry in entries:
                        ...
                        # 6: time_last
                        update_hijack_withdrawals.add((entry[2],
                                                       withdrawal[3]))
                        if entry[5] >= withdrawal[2]:
                            continue
                        # matching withdraw with a hijack
                        if withdrawal[1] not in entry[1] and withdrawal[
                                1] in entry[0]:
        ...
        def _update_bgp_updates(self):
            ...
            for hijack_key in self.insert_hijacks_entries:
                for bgp_entry_to_update in self.insert_hijacks_entries[
            ...
            if self.handled_bgp_entries:
                try:
                    ...
                    log.exception('handled bgp entries {}'.format(
                        self.handled_bgp_entries))
                    return -1
            ...
            try:
                ...
                for key in self.insert_hijacks_entries:
                    entry = (
                        ...
                        datetime.datetime.fromtimestamp(
                            self.insert_hijacks_entries[key]
                            ['timestamp_of_config']),  # timestamp_of_config
                        '',  # comment
                        # peers_seen
        ...
        def _scheduler_instruction(self, message):
            ...
            if msg_['op'] == 'bulk_operation':
                ...
                log.warning(
                    'Received uknown instruction from scheduler: {}'.format(
                        msg_))
        ...
        def _save_config(self, config_hash, yaml_config, raw_config, comment):
            try:
                ...
                with get_wo_cursor(self.wo_conn) as db_cur:
                    db_cur.execute(
                        query, (config_hash, json.dumps(yaml_config),
                                raw_config, datetime.datetime.now(), comment))
            except Exception:
