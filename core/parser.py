...
class ConfParser():
	...
	def parse_file(self):
		...
		sections_def_list = [section for section in sections_list if section in self.section_groups]
		sections_other_list = [section for section in sections_list if section not in self.section_groups]
			...
			if(self.validate_options(section_name)):
				...
				for field in fields:
					...
					if(type_of_field not in self.supported_fields):
						self.raise_error('field-wrong', section_name, type_of_field)
		...
		for group in self.section_groups:
			...
			if group in section_labels:
				...
				for field in fields:
					...
					values = field[1]
					if(group == 'asns_group'): 
						ret_[group][label] = self.process_group[group](values, group, definition=True)

					else:
						ret_[group][label] = self.process_group[group](values, group, label=label, definition=True)
	...
	def process_field__asns(self, field, where, definition=False):
		try:
			if(definition == True):
				list_of_asns = list(map(int, ''.join(field.split()).split(',')))
					...
					if(set(riperis_).issubset(self.available_ris)):
						...
						for ris_monitor in riperis_:
							if(ris_monitor in self.available_ris):
								...
								print("Warning ", ris_monitor, " is not available.")
						
						return list_

				elif(label == 'bgpmon'):
					bgpmon_ = (''.join(field.split())).lstrip('(').rstrip(')').split(':')
					
					if(len(bgpmon_) > 2):
						print("Error only one value expected at ", label, "in", where, ".") 
						self.valid=False
	...
	def raise_error(self, type_of_error, where, field=None):
		...
		if(type_of_error == "keyword-missing"):
			print("Error -> Missing keyword 'prefixes' or 'origin_asns' on config block: ", where)
			self.valid = False

		elif(type_of_error == "field-wrong"):
			print("Error -> Found in '", where, "' the field -> '", field,  " ' which is not supported.")
			print("List of supported fields: ", self.supported_fields)
			self.valid = False

		elif(type_of_error == "mitigation-error-type"):
			print("Error -> Found in '", where, "' the mitigation field has an unsupported value.")
