...
class SQLite_db(Process):
    ...
    class Worker(ConsumerProducerMixin):
        ...
        def __init__(self, connection):
            ...
            # DB variables
            self.db_filepath = './db/artemis.db'
            self.db_conn = None
            self.db_cur = None
        ...
        def handle_bgp_update(self, message):
            ...
            extract_msg = (msg_['prefix'], msg_['key'], str(msg_['path'][-1]), str(msg_['peer_asn']), str(msg_['path']), msg_['service'], \
                msg_['type'], str(msg_['communities']), float(msg_['timestamp']), 0, 0, self.find_best_prefix_match(msg_['prefix']) )
        ...
        def handle_handled_bgp_update(self, message):
            ...
            extract_msg = (msg_['prefix'], str(msg_['path'][-1]), str(msg_['peer_asn']), str(msg_['path']), msg_['service'], \
                msg_['type'], str(msg_['communities']), float(msg_['timestamp']), 0, 1, self.find_best_prefix_match(msg_['prefix']), msg_['key'] )
        ...
        def handle_config_notify(self, message):
            log.info(' [x] SQLite_db - Config Notify')
            raw = message.payload
        ...
        def handle_config_request_reply(self, message):
            log.info(' [x] Sqlite_db - Received Configuration')
        ...
        def create_tables(self):
            bgp_updates_table = "CREATE TABLE IF NOT EXISTS 'bgp_updates' ( " + \
                ...
                "'service'   VARCHAR ( 50 ), " + \
                "'type'  VARCHAR ( 1 ), " + \
                "'communities'  VARCHAR ( 480 ), " + \
                "'timestamp' REAL, " + \
                "'hijack_ckey' VARCHAR ( 20 ), " + \
                "'handled'   BOOLEAN, " + \
                "'matched_prefix' VARCHAR ( 44 ))"
        ...
        def create_connect_db(self):
            try:
                ...
                log.info('Error on SQLite_db creation/connection: ' + str(e))
            finally:
                log.info('SQLite DB created/connected..')
        ...
        def _insert_bgp_updates(self):
            try:
                self.db_cur.executemany("INSERT INTO bgp_updates (prefix, ckey, origin_as, peer_as, as_path, service, type, communities, " + \
                    "timestamp, hijack_ckey, handled, matched_prefix) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", self.insert_bgp_entries)
                self.db_conn.commit()
            except:
        ...
        def _update_bgp_updates(self):
            ...
            for hijack_ckey in self.tmp_hijacks_dict:
                for bgp_entry_to_update in self.tmp_hijacks_dict[hijack_ckey]['monitor_keys']:
                    num_of_updates += 1
                    self.update_bgp_entries.append((1, str(hijack_ckey), bgp_entry_to_update))
            
            if len(self.update_bgp_entries) > 0:
                try:
                    self.db_cur.executemany("UPDATE bgp_updates SET handled=?, hijack_ckey=? WHERE ckey=? ", self.update_bgp_entries)
                    self.db_conn.commit()
                except:
                    log.info("error on 1_update_bgp_updates!!")
                    self.db_conn.rollback()
                    return -1
            ...
            if len(self.handled_bgp_entries) > 0:
                try:
                    self.db_cur.executemany("UPDATE bgp_updates SET prefix=?, origin_as=?, peer_as=?, as_path=?, service=?, type=?, communities=?, timestamp=?, hijack_ckey=?, handled=?, matched_prefix=? " \
                        + " WHERE ckey=?", self.handled_bgp_entries)
                    self.db_conn.commit()
                except:
                    log.info("error on 2_update_bgp_updates!!")
                    self.db_conn.rollback()
                    return -1
        ...
        def _insert_update_hijacks(self):
            for ckey in self.tmp_hijacks_dict:
                # Current SQLite
                try:
                    cmd_ = "INSERT OR IGNORE INTO hijacks (ckey, type, prefix, hijack_as, num_peers_seen, num_asns_inf, "
                    ...
                    cmd_ += str(self.tmp_hijacks_dict[ckey]['time_last']) + ",0,0,0)"

                    self.db_cur.execute(cmd_)
                    self.db_conn.commit()

                    cmd_ = "UPDATE hijacks SET num_peers_seen=" + str(len(self.tmp_hijacks_dict[ckey]['peers_seen'])) + ", num_asns_inf=" + str(len(self.tmp_hijacks_dict[ckey]['inf_asns']))
                    cmd_ += ", time_started=" + str(self.tmp_hijacks_dict[ckey]['time_started']) + ", time_last=" + str(self.tmp_hijacks_dict[ckey]['time_last'])
                    cmd_ += " where ckey='" + str(ckey) + "'"
                    self.db_cur.execute(cmd_)
                    self.db_conn.commit()
                    #try:
                    """ For Sqlite 3.24
                    ...
                    cmd_ += str(len(self.tmp_hijacks_dict[ckey]['peers_seen'])) + "," + str(len(self.tmp_hijacks_dict[ckey]['inf_asns'])) + "," + str(self.tmp_hijacks_dict[ckey]['time_started']) + "," 
                    cmd_ += str(self.tmp_hijacks_dict[ckey]['time_last']) + ",0,0,0)"
                    cmd_ += " ON CONFLICT(ckey) "
                    cmd_ += "DO UPDATE SET num_peers_seen=" + str(len(self.tmp_hijacks_dict[ckey]['peers_seen'])) + ", num_asns_inf=" + str(len(self.tmp_hijacks_dict[ckey]['inf_asns']))
                    cmd_ += ", time_started=" + str(self.tmp_hijacks_dict[ckey]['time_started']) + ", time_last=" + str(self.tmp_hijacks_dict[ckey]['time_last'])
                    cmd_ += " where ckey='" + str(ckey) + "'"
                    """
                    # For normal SQL (Sqlite doesn't support rowcount)
                    """  
                    cmd_ = "UPDATE hijacks SET num_peers_seen=" + str(len(self.tmp_hijacks_dict[ckey]['peers_seen'])) + ", num_asns_inf=" + str(len(self.tmp_hijacks_dict[ckey]['inf_asns']))
                    cmd_ += ", time_started=" + str(self.tmp_hijacks_dict[ckey]['time_started']) + ", time_last=" + str(self.tmp_hijacks_dict[ckey]['time_last'])
                    cmd_ += " where ckey='" + str(ckey) + "'"
                    ...
                    log.info("error on _insert_update_hijacks!!")
                    self.db_conn.rollback()
                    return -1
