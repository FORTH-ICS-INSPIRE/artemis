        ...
        try:
            ...
            response = requests.get(
                url=url_, headers={
                    "Prefer": "count=exact"})
            if 'Content-Range' in response.headers:
                self.total_bgp_unhandled_updates = int(
                    response.headers['Content-Range'].split('/')[1])
        except BaseException:
        ...
        try:
            ...
            response = requests.get(
                url=url_, headers={
                    "Prefer": "count=exact"})
            if 'Content-Range' in response.headers:
                self.total_hijacks_resolved = int(
                    response.headers['Content-Range'].split('/')[1])
        except BaseException:
        ...
        try:
            ...
            response = requests.get(
                url=url_, headers={
                    "Prefer": "count=exact"})
            if 'Content-Range' in response.headers:
                self.total_hijacks_under_mitigation = int(
                    response.headers['Content-Range'].split('/')[1])
        except BaseException:
        ...
        try:
            ...
            response = requests.get(
                url=url_, headers={
                    "Prefer": "count=exact"})
            if 'Content-Range' in response.headers:
                self.total_hijacks_ignored = int(
                    response.headers['Content-Range'].split('/')[1])
        except BaseException:
