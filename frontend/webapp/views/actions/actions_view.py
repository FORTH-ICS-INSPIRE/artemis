...
def submit_new_comment():
    ...
    hijack_key = request.values.get('hijack_key')
    app.artemis_logger.debug('hijack_key: {0} new_comment: {1}'.format(
        hijack_key, new_comment))
...
def set_new_password():
    ...
    if form.validate_on_submit():
        if form.old_password.data is not None:
            app.artemis_logger.debug('verify: {}'.format(
    ...
    return render_template(
        'new_password.htm', password_change=form, status=_status)
