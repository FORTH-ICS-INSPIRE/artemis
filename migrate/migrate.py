...
def migrate(dest_version, db_cur):
    print("Executing migration {}..".format(dest_version['db_version']))
    print("{0}".format(dest_version['description']))
    migration_command = read_migration_sql_file(dest_version['file'])
...
def update_version(version, db_cur):
    cmd = "UPDATE db_details SET version={0} WHERE ID={1};".format(version, 0)
    try:
        db_cur.execute(cmd)
        db_cur.commit()
    except Exception:
...
def start_migrations(db_version, target_version, db_conn):
    ...
    while(current_version != target_version):
        ...
        if next_version in migration_data:
            status = migrate(migration_data[str(next_version)], db_cur)
            if status:
                update_version(current_version, db_cur)
        else:
...
if __name__ == "__main__":
    ...
    if target_version != db_version:
        ...
        msg += "Migrating from version {0} to {1}".format(db_version, target_version)
        print(msg)
        result = start_migrations(db_version, target_version, db_conn)
