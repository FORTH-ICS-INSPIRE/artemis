...
def main():
    parser = argparse.ArgumentParser(
        description="analyze the BGP records of BGPStream")
    parser.add_argument(
        '-i',
        '--input_file',
        dest='input_file',
    ...
    end_time = format_timestamp(
        int(filename_elems[2].split('_')[1].split('.csv')[0]))
        ...
        for row in csv_reader:
            if len(row) == 8:
                ...
                if origin_asn != '':
                    ...
                    observed_prefixes[prefix]['origins'][origin_asn].add(
                        timestamp)
                ...
                if len(row[2]) > 1:
                    ...
                    observed_prefixes[prefix]['first_hops'][first_hop].add(
                        timestamp)

    for prefix in observed_prefixes:
        for origin_asn in observed_prefixes[prefix]['origins']:
            observed_prefixes[prefix]['origins'][origin_asn] = sorted(
                list(observed_prefixes[prefix]['origins'][origin_asn]))
            for i, timestamp in enumerate(
                    observed_prefixes[prefix]['origins'][origin_asn]):
                observed_prefixes[prefix]['origins'][origin_asn][i] = format_timestamp(
                    timestamp)

        for first_hop in observed_prefixes[prefix]['first_hops']:
            observed_prefixes[prefix]['first_hops'][first_hop] = sorted(
                list(observed_prefixes[prefix]['first_hops'][first_hop]))
            for i, timestamp in enumerate(
                    observed_prefixes[prefix]['first_hops'][first_hop]):
                observed_prefixes[prefix]['first_hops'][first_hop][i] = format_timestamp(
                    timestamp)

    if not os.path.isdir(args.output_dir):
        os.mkdir(args.output_dir)
    out_file = '{}/{}_analyzed.json'.format(args.output_dir,
                                            args.input_file.split('/')[-1].split('.csv')[0])

    with open(out_file, 'w') as f:
        json.dump(observed_prefixes, f)

    print(
        'REPORT FROM {} TO {} generated on {}'.format(
            start_time,
            end_time,
            out_file))
