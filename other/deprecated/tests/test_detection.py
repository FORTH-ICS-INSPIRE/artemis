...
class TestDetection(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # prefix: dead:beef::/32, origin_asns = {1}, neighbors = {2,3,4}
        node = self.detection.prefix_tree.add('dead:beef::/32')
        node.data['confs'] = []
        conf_obj = {'origin_asns': {1}, 'neighbors': {2, 3, 4}}
        node.data['confs'].append(conf_obj)

        # prefix: dead:beef::/32, origin_asns = {15}, neighbors = {16,17,18}
        conf_obj = {'origin_asns': {15}, 'neighbors': {16, 17, 18}}
        node.data['confs'].append(conf_obj)
    ...
    def test_type01_detection(self, mock_init):
        ...
        self.assertEqual(
            len(monitor_rows),
            3,
            msg='[!] Wrong number of monitor entries')
        self.assertEqual(
            len(hijack_rows),
            2,
            msg='[!] Wrong number of detected hijacks')

        # check types
        self.assertEqual(
            hijack_rows[0][hij_fields['type']], '0', msg='[!] Wrong hijack type detected')
        self.assertEqual(
            hijack_rows[1][hij_fields['type']], '1', msg='[!] Wrong hijack type detected')

        # check monitors updated hijack id
        self.assertEqual(monitor_rows[0][mon_fields['hijack_id']],
                         None, msg='[!] Legal Monitor entry has Hijack ID set')
        self.assertEqual(monitor_rows[1][mon_fields['hijack_id']],
                         hijack_rows[0][hij_fields['id']],
                         msg='[!] Malicious Monitor entry has no Hijack ID')
        self.assertEqual(monitor_rows[2][mon_fields['hijack_id']],
                         hijack_rows[1][hij_fields['id']],
                         msg='[!] Malicious Monitor entry has no Hijack ID')

        # check if monitors are handled
        for monitor_entry in monitor_rows:
            self.assertTrue(
                monitor_entry[-1], msg='[!] The monitor should have been handled')
        ...
        self.assertEqual(
            len(hijack_rows),
            2,
            msg='[!] Wrong number of hijacks detected')

        self.assertEqual(hijack_rows[0][hij_fields['time_last_updated']],
                         666, msg='[!] Hijack timstamp not updated')
        self.assertEqual(hijack_rows[0][hij_fields['num_asns_inf']],
                         12, msg='[!] Wrong number of infected ASNs')
    ...
    def test_subprefix_type01_detection(self, mock_init):
        ...
        self.assertEqual(
            hijack_rows[0][hij_fields['type']], '0', msg='[!] Wrong hijack type detected')
        self.assertEqual(
            hijack_rows[1][hij_fields['type']], '0', msg='[!] Wrong hijack type detected')
        ...
        # check if hijack fields are updated correctly
        self.assertEqual(hijack_rows[2][hij_fields['time_last_updated']],
                         300, msg='[!] Hijack timstamp not updated')
        self.assertEqual(hijack_rows[2][hij_fields['num_asns_inf']],
                         1, msg='[!] Wrong number of infected ASNs')
    ...
    def test_subprefix_detection(self, mock_init):
        ...
        # check types
        self.assertEqual(
            hijack_rows[0][hij_fields['type']], 'S', msg='[!] Wrong hijack type detected')
    ...
    @mock.patch.object(Detection, 'init_detection')
    def test_type01_detection_ipv6(self, mock_init):
        from webapp.data.models import Monitor
    ...
    @mock.patch.object(Detection, 'init_detection')
    def test_prepending_check_ipv6(self, mock_init):
        from webapp.data.models import Monitor
    ...
    @mock.patch.object(Detection, 'init_detection')
    def test_subprefix_type01_detection_ipv6(self, mock_init):
        from webapp.data.models import Monitor
    ...
    @mock.patch.object(Detection, 'init_detection')
    def test_superprefix_ipv6(self, mock_init):
        from webapp.data.models import Monitor
    ...
    @mock.patch.object(Detection, 'init_detection')
    def test_subprefix_detection_ipv6(self, mock_init):
        from webapp.data.models import Monitor
    ...
    @mock.patch.object(Detection, 'init_detection')
    def test_multiple_confs_per_prefix_ipv6(self, mock_init):
        from webapp.data.models import Monitor
