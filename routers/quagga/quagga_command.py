...
def main():
    parser = argparse.ArgumentParser(
        description="send a prefix announce(/removal) command via telnet to BGP router (quagga)")
    parser.add_argument('-th', '--thost', dest='telnet_host',
                        type=str, help='host to telnet', default='localhost')
    # https://www.systutorials.com/docs/linux/man/8-bgpd/
    parser.add_argument('-tp', '--tport', dest='telnet_port',
                        type=int, help='port to telnet', default=2605)
    parser.add_argument('-la', '--lasn', dest='local_asn',
                        type=int, help='local AS of BGP speaker', required=True)
    parser.add_argument('-ap', '--aprefix', dest='announce_prefix',
                        type=str, help='prefix to announce', required=True)
    parser.add_argument('-w', '--withdraw', dest='withdraw_prefix',
                        help='withdraw prefix', action='store_true')
    ...
    if is_valid_ipv4_prefix(args.announce_prefix):
        ...
        if not args.withdraw_prefix:
            ...
            info_str += "Withdrew"
        print("{} prefix '{}' from AS {}".format(
            info_str, args.announce_prefix, args.local_asn))
    else:
